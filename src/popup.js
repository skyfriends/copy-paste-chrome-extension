window.addEventListener('load', () => {
  ...
  chrome.tabs.query({ active: true }, (res) => {
    ...
    chrome.storage.local.get('passed', (data) => {
      ...
      copiedData.push(
        copiedData[copiedData.length - 1]
          .match(/[0-9]/gi)
          .slice(0, 5)
          .join(''),
      );
      ...
      pasta.addEventListener('click', (e) => {
        if (currentTab.includes('gearbubble')) {
          const firstGB = document.getElementById('dropship_order_first_name');
          const lastGB = document.getElementById('dropship_order_last_name');
          const firstAddGB = document.getElementById('dropship_order_address1');
          const secondAddGB = document.getElementById('dropship_order_address2');
          const cityGB = document.getElementById('dropship_order_city');
          const zipGB = document.getElementById('dropship_order_zip_code');
          const stateGB = document.getElementById('states_select');
    ...
    .join('')}"`,
            ...
            zipGB.value =
              "${copiedData[2].match(/[0-9]/gi).slice(0, 5).join('')}";
            stateGB.value =
              "${copiedData[2].split(',')[1].match(/[A-Z]/gi).join('')}";
            //     `,
            //           });
          }
        }
        if (currentTab.includes('gooten')) {
          ...
          const stateSelectBox = document.getElementsByTagName('select')[3];
